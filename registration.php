...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if(strlen($user_firstname) < 4) {
        $error['user_firstname'] = 'Firstname needs to be longer';
    }
    ...
    if(strlen($user_lastname) < 4) {
        $error['user_lastname'] = 'Lastname needs to be longer';
    }
    ...
    if(email_exists($email)) {
        $error['email'] = 'Email already exists, <a href="index.php" title="">Please login</a>';
    }
                ...
                <h1>Register</h1>
                    <form role="form" action="registration.php" method="post" id="login-form" autocomplete="off">
                        <div class="form-group">
                            ...
                            <input type="text" name="user_firstname" id="user_firstname" class="form-control" placeholder="Enter Firstname" autocomplete="on" value="<?php echo isset($user_firstname) ? $user_firstname : ''; ?>">
                            
                            <p class="danger"><?php echo isset($error['user_firstname']) ? $error['user_firstname'] : ''; ?></p>

                        </div>
                        <div class="form-group">
                            <label for="user_lastname" class="sr-only">user_lastname</label>
                            <input type="text" name="user_lastname" id="user_lastname" class="form-control" placeholder="Enter Lastname" autocomplete="on" value="<?php echo isset($user_lastname) ? $user_lastname : ''; ?>">
